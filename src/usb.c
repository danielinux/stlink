...
stlink_t *stlink_open_usb(enum ugly_loglevel verbose, bool reset, char serial[16])
{
    ...
    while (cnt--) {
        ...
        if ((desc.idProduct == STLINK_USB_PID_STLINK_32L) || (desc.idProduct == STLINK_USB_PID_STLINK_NUCLEO) || (desc.idProduct == STLINK_USB_PID_STLINK_V3)) {
    ...
    if ((desc.idProduct == STLINK_USB_PID_STLINK_NUCLEO) || desc.idProduct == STLINK_USB_PID_STLINK_V3) {
...
static size_t stlink_probe_usb_devs(libusb_device **devs, stlink_t **sldevs[]) {
    ...
    while ((dev = devs[i++]) != NULL) {
        ...
        if (desc.idProduct != STLINK_USB_PID_STLINK_32L &&
            ...
            desc.idProduct != STLINK_USB_PID_STLINK_V3)
            continue;
